...
def parse(p):
    ...
    if DHCP in p:
        ...
        try:
            ...
            log_message(domains)  # Записываем домены в лог
        except:
            pass
...
def alert(new_dhcp_servers):
    ...
    log_message(f"[!] DHCP rogue: {dhcp_servers}")  # Записываем предупреждение в лог
    system("zenity --warning --title='DHCP rogue server' --text='DHCP rogue: %s' &" % dhcp_servers)
...
while True:
    ...
    if not dhcp_servers_legal:
        ...
        log_message("[*] DHCP legal: " + ", ".join(map(str, dhcp_servers_legal)))  # Записываем легальные серверы в лог
